...
class RepresentationEvaluator(BaseEvaluator):
    ...
    def _evaluate(self, task_id: int) -> List[float]:
        ...
        # need feats according to self.dim
        feats = torch.zeros([self.eval_scenario.n_samples, 2, self.dim], dtype=torch.float32).to(self.device)
    ...
    def on_eval_end(self, tasks_acc: float, tasks_auc: float, current_task_id: int) -> None:
        ...
        # )
        msg = "\n" + f"Representation | Observed {self.name}-Acc: {tasks_acc:.2f} | Observed {self.name}-Auc: {tasks_auc:.2f} |"
